		...
		<form class="layui-form"  lay-filter="dataFrm" id="dataFrm">
			<div class="layui-form-item">
				...
				<div class="layui-inline">
					<label class="layui-form-label">学生姓名:</label>
					<div class="layui-input-inline">
						<input type="text" name="custname" lay-verify="required"   placeholder="请输入学生姓名" autocomplete="off"
							class="layui-input">
					</div>
				</div>
			</div>
			<div class="layui-form-item">
				<div class="layui-inline">
					<label class="layui-form-label">学生电话:</label>
					<div class="layui-input-inline">
						<input type="text" name="phone"  placeholder="请输入学生电话" autocomplete="off"
							class="layui-input">
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label">学生地址:</label>
					<div class="layui-input-inline">
						<input type="text" name="address"  placeholder="请输入学生地址" autocomplete="off"
							class="layui-input">
					</div>
				</div>
			</div>
			<div class="layui-form-item">
				<div class="layui-inline">
					<label class="layui-form-label">学生年龄:</label>
					<div class="layui-input-inline">
						<input type="text" name="career"   placeholder="请输入学生年龄" autocomplete="off"
							class="layui-input">
					</div>
				</div>
				<div class="layui-inline">
	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/customer/loadAllCustomer.action' //数据接口
			    ...
			    ,cols: [[   //列表数据
			     {type: 'checkbox', fixed: 'left'}
			      ...
			      ,{field:'custname', title:'学生姓名',align:'center',width:'100'}
			      ,{field:'phone', title:'学生电话',align:'center',width:'120'}
			      ,{field:'address', title:'学生地址',align:'center',width:'150'}
			      ,{field:'career', title:'学生年龄',align:'center',width:'120'}
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${cb}/customer/loadAllCustomer.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
		   ...
		   table.on('tool(customerTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.custname+'】这个学生吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/customer/deleteCustomer.action",{identity:data.identity},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openAddCustomer(){
				mainIndex=layer.open({
					...
					success:function(index){
						//清空表单数据       
						$("#dataFrm")[0].reset();
						url="${cb}/customer/addCustomer.action";
					}
				});
			}
			//打开修改页面
			function openUpdateCustomer(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="${cb}/customer/updateCustomer.action";
					}
				});
			}
			//保存
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些学生吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/customer/deleteBatchCustomer.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
