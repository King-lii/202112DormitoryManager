...
<body class="childrenBody">
		 ...
		 <div class="layui-form-item">
		    <div class="layui-inline">
		     ...
		     <div class="layui-inline">
		      ...
		      <div class="layui-input-inline">
		       		 <input type="radio" name="isrenting" value="1" title="已出售">
					 <input type="radio" name="isrenting" value="0" title="未出售">
		      </div>
		    </div>
		 </div>
				...
				<div class="layui-row layui-col-space10">
					<div class="layui-col-md9 layui-col-xs7">
						<div class="layui-form-item magt3">
							<label class="layui-form-label">课程编号:</label>
							<div class="layui-input-block">
								<input type="text" name="carnumber" id="carnumber" class="layui-input" lay-verify="required" placeholder="请输入课程编号">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">课程名称:</label>
							<div class="layui-input-block">
								<input type="text" name="cartype" class="layui-input" lay-verify="required" placeholder="请输入课程名称">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">课程难度:</label>
							<div class="layui-input-block">
								<input type="text" name="color" class="layui-input" lay-verify="required" placeholder="请输入课程难度">
							</div>
						</div>
					</div>
				...
				<div class="layui-form-item magb0">
					<div class="layui-inline">
						<label class="layui-form-label">课程总价:</label>
							<div class="layui-input-block">
								<input type="text" name="price" class="layui-input" lay-verify="required|number" placeholder="请输入课程总价">
							</div>
					</div>
					<div class="layui-inline">
						<label class="layui-form-label">课程单价:</label>
							<div class="layui-input-block">
								<input type="text" name="rentprice" class="layui-input" lay-verify="required|number" placeholder="请输入课程单价">
							</div>
					</div>
					<div class="layui-inline">
						<label class="layui-form-label">课程数量:</label>
							<div class="layui-input-block">
								<input type="text" name="deposit" class="layui-input" lay-verify="required|number" placeholder="请输入课程数量">
							</div>
					</div>
				</div>
				<div class="layui-form-item magb0">
					<label class="layui-form-label">课程描述:</label>
						<div class="layui-input-block">
							<input type="text" name="description" class="layui-input" lay-verify="required" placeholder="请输入课程描述">
						</div>
				</div>
				<div class="layui-form-item magb0">
					<label class="layui-form-label">出售状态:</label>
						<div class="layui-input-block">
						<input type="radio" name="isrenting" value="1" title="已出售">
						 <input type="radio" name="isrenting" value="0" title="未出售" checked="checked">
					</div>
				</div>
	...
	<div id="viewCarImageDiv" style="display: none;text-align: center;">
		<img alt="课程图片" width="550" height="350" id="view_carimg">
	</div>
	<!-- 查看大图弹出的层 结束 -->
	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','upload'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/car/loadAllCar.action' //数据接口
			    ...
			    ,cols: [[   //列表数据
			     {type: 'checkbox', fixed: 'left'}
			      ,{field:'carnumber', title:'课程编号',align:'center',width:'120'}
			      ,{field:'cartype', title:'课程名称',align:'center',width:'100'}
			      ,{field:'color', title:'课程难度',align:'center',width:'120'}
			      ,{field:'price', title:'课程总价',align:'center',width:'150'}
			      ,{field:'rentprice', title:'课程单价',align:'center',width:'120'}
			      ,{field:'deposit', title:'课程数量',align:'center',width:'120'}
			      ,{field:'isrenting', title:'出售状态',align:'center',width:'100',templet:function(d){
			    	  return d.isrenting=='1'?'<font color=blue>已出售</font>':'<font color=red>未出售</font>';
			      }}
			      ,{field:'description', title:'课程描述',align:'center',width:'180'}
			      ,{field:'carimg', title:'课程图片',align:'center',width:'120',templet:function(d){
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${cb}/car/loadAllCar.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
		   ...
		   table.on('tool(carTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.carnumber+'】这个课程吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/car/deleteCar.action",{carnumber:data.carnumber},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些课程吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/car/deleteBatchCar.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
