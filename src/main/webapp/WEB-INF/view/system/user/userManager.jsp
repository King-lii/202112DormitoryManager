	    ...
	    layui.extend({
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/user/loadAllUser.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${cb}/user/loadAllUser.action?"+params
				})
			});
		   ...
		   table.on('tool(userTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.realname+'】这个用户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/user/deleteUser.action",{userid:data.userid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			     ...
			     openUpdateUser(data);
				   ...
				   layer.confirm('真的重置【'+data.realname+'】这个用户的密码吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/user/resetUserPwd.action",{userid:data.userid},function(res){
				    	   layer.msg(res.msg);
				       })
				     }); 
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些用户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/user/deleteBatchUser.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openselectUserRole(data){
				mainIndex=layer.open({
					...
					yes:function(index, layero){
					    ...
					    //保存
					    $.post("${cb}/user/saveUserRole.action",params,function(obj){
					    	layer.msg(obj.msg);
					    })
					},
					success:function(index){
						//渲染数据表格
						 var roleTableIns=table.render({
						    ...
						    ,url:'${cb}/user/initUserRole.action?userid='+data.userid //数据接口
						    ,title: '角色列表'//数据导出来的标题
						    ,cellMinWidth:100 //设置列的最小默认宽度
						    ,cols: [[   //列表数据
						     {type: 'checkbox', fixed: 'left'}
