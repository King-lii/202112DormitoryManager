...
<body class="childrenBody">
	...
	<script type="text/javascript">
	    ...
	    layui.extend({
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/menu/loadAllMenu.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				//alert(params);调试测试
				tableIns.reload({
					url:"${cb}/menu/loadAllMenu.action?"+params
				})
			});
		   ...
		   table.on('tool(menuTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  //先判断当前菜单有没有子节点
				  $.post("${cb}/menu/checkMenuHasChildren.action?id="+data.id,function(obj){
					  if(obj.code>=0){
						  layer.msg("当前菜单有子节点同，请先删子节点");
					  }else{
						  layer.confirm('真的删除【'+data.title+'】这个菜单吗', function(index){
						       //向服务端发送删除指令
						       $.post("${cb}/menu/deleteMenu.action",{id:data.id},function(res){
						    	   layer.msg(res.msg);
						    	    //刷新数据 表格
									tableIns.reload();
									//刷新左边的树
			...
			var menuTree = dtree.render({
		          ...
		          url: "${cb}/menu/loadMenuManagerLeftTreeJson.action?spread=1",  // 使用url加载（可与data加载同时存在）
				  icon: "2",
				  accordion:true
				});
		...
		function reloadTable(id){
			tableIns.reload({
				url:"${cb}/menu/loadAllMenu.action?id="+id
			})
		}
	</script>
</body>
