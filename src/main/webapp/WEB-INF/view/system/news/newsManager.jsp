	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','laydate','layedit' ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/news/loadAllNews.action' //数据接口
		   ...
		   table.on('tool(newsTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.title+'】这个公告吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/news/deleteNews.action",{id:data.id},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些公告吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/news/deleteBatchNews.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
