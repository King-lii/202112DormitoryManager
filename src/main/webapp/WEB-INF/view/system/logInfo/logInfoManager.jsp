	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','laydate'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/logInfo/loadAllLogInfo.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${cb}/logInfo/loadAllLogInfo.action?"+params,
					page:{curr:1}
		   ...
		   table.on('tool(logInfoTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.loginname+'】这个日志吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/logInfo/deleteLogInfo.action",{id:data.id},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些日志吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/logInfo/deleteBatchLogInfo.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
