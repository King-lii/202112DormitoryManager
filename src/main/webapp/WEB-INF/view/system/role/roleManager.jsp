...
<head>
	...
	<link rel="stylesheet" href="${cb }/resources/layui_ext/dtree/dtree.css">
	<link rel="stylesheet" href="${cb }/resources/layui_ext/dtree/font/dtreefont.css">
</head>
<body class="childrenBody">
	...
	<script type="text/javascript">
	    var tableIns;
	    layui.extend({
			dtree:'${cb}/resources/layui_ext/dist/dtree'
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${cb}/role/loadAllRole.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${cb}/role/loadAllRole.action?"+params
				})
			});
		   ...
		   table.on('tool(roleTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.rolename+'】这个角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/role/deleteRole.action",{roleid:data.roleid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("${cb}/role/deleteBatchRole.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openselectRoleMenu(data){
				...
				mainIndex=layer.open({
					...
					yes:function(index, layero){
						...
						//保存角色和菜单的关系
						$.post("${cb}/role/saveRoleMenu.action",params,function(obj){
							layer.msg(obj.msg);
						})
					},
					success:function(index){
					   ...
					   menuTree = dtree.render({
			              ...
			              checkbarData: "choose" ,
				          url: "${cb}/role/initRoleMenuTreeJson.action?roleid="+data.roleid // 使用url加载（可与data加载同时存在）
					    });
					}
				});
			}
